    ...
    <div class="container">
        <div class="col-12 text-center">
            <div class="alert alert-success" role="alert">
                <h1 class="mb-0" style="color: green;">Nueva Registro de viaje</h1>
            </div>
        </div>
        <p></p>
        <p></p>
        <form @submit.prevent="saveRegistros_viaje">
    ...
    data() {
        return {
            registros_viaje: {
                ...
                fecha_fin: '',
                kilometraje: '',
            }, vehiculos: [], id: '0', conductores: [], id: '0', rutas: [], id: '0'
        }
    },
    methods: {
        cancelar() {
        ...
        async saveRegistros_viaje() {
            const res = await axios.post(`http://127.0.0.1:8000/api/registros_viajes/`, this.registros_viaje)
            if (res.status == 200) {
                this.$router.push({ name: 'Registros_viajes' })
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                    title: 'Registros_viaje has been saved',
