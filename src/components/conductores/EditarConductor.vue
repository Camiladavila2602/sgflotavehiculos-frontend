    ...
    <div class="container">
        <div class="col-12 text-center">
            <div class="alert alert-success" role="alert">
                <h1 class="mb-0" style="color: green;">Editar Conductor</h1>
            </div>
        </div>
        <p></p>
        <p></p>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="date" class="form-control" id="fecha_contratacion"
                                aria-describedby="codigoHelp" name="fecha_contratacion"
                                v-model='conductor.fecha_contratacion'>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async updateConductor() {
            const res = await axios.put(`http://127.0.0.1:8000/api/conductores/${this.conductor.id}`, this.conductor)
            if (res.status == 200) {
                this.$router.push({ name: 'Conductores' })
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                    title: 'Conductor has been updated',
    ...
    mounted() {
        this.conductor.id = this.$route.params.id;
        console.log(`http://127.0.0.1:8000/api/conductores/${this.conductor.id}`)
        axios
            .get(`http://127.0.0.1:8000/api/conductores/${this.conductor.id}`)
            .then(response => { this.conductor = response.data.conductor })
    },
}
</script>