    ...
    <div class="container">
        <div class="col-12 text-center">
            <div class="alert alert-success" role="alert">
                <h1 class="mb-0" style="color: green;">Nueva Ruta</h1>
            </div>
        </div>
        <p></p>
        <p></p>
        <form @submit.prevent="saveRuta">
...
export default {
    ...
    methods: {
        ...
        async saveRuta() {
            const res = await axios.post(`http://127.0.0.1:8000/api/rutas/`, this.ruta)
            if (res.status == 200) {
                this.$router.push({ name: 'Rutas' })
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                    title: 'Ruta has been saved',
